...
public class DictionaryTest {
	...
	dict.load(input);
	Assert.assertEquals(9123, dict.size()); // out of the 10,000 input
						// words, there are only 9123
						// unique words
	...
	while (sc.hasNext())
	    dict.delete(sc.next());
	sc.close();
	Assert.assertEquals(9123 - 290, dict.size()); // out of the 10,000 input
						      // words, there are only
						      // 9123 unique words
